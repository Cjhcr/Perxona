...
## Deploying the interview coach

The interview experience is served by the Express app, so deploy it to a Node.js host such as Render, Railway, Fly.io, or
another service that supports a long-running Node process:

1. Set the service root directory to `samples/express`.
2. Use `npm ci` as the build command and `npm start` as the start command.
3. Set `NODE_ENV=production` and configure every required variable from `.env.example` in the host's secret manager.
4. Keep `PERXONA_CONNECT_SECRET_KEY` server-side. Add the deployed hostname to the publishable key's allowed-domain list.
5. Keep the service private until request authentication is added: the chatbot routes spend account credits and are
   intentionally unauthenticated in this hackathon sample. A per-IP chat ceiling is enabled by default; adjust it with
   `CHAT_RATE_LIMIT_PER_MINUTE`, set `TRUST_PROXY_HOPS` for the hosting provider's proxy chain, and add real authentication
   before a public launch.

The host must provide Node 22 or newer and a `PORT` environment variable. The server already reads `PORT` and serves the
frontend and API from the same origin, so no separate static deployment or CORS configuration is required.
