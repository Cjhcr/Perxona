...
const CHAT_RATE_LIMIT_MAX = Math.max(
  1,
  Number.parseInt(process.env.CHAT_RATE_LIMIT_PER_MINUTE || "12", 10) || 12,
);
...
function chatRateLimit(req, res, next) {
  ...
  res.setHeader("RateLimit-Limit", CHAT_RATE_LIMIT_MAX);
  res.setHeader(
    "RateLimit-Remaining",
    Math.max(0, CHAT_RATE_LIMIT_MAX - bucket.count),
  );
  ...
  if (chatRateLimitBuckets.size > 1_000) {
    for (const [bucketKey, value] of chatRateLimitBuckets) {
      if (value.resetAt <= now) chatRateLimitBuckets.delete(bucketKey);
    }
  }
  next();
}
